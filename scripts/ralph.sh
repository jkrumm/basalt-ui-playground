#!/usr/bin/env bash
# CBBI Blueprint — Bun Monorepo Reboot — RALPH Loop Runner
#
# Usage:
...
GROUP_TITLES=(
  ...
  "Elysia API Server (Core)"
  "Database + Drizzle v1 Beta"
  "BetterAuth + Elysia Integration"
  "OpenTelemetry (API Layer)"
  "TanStack Start Frontend (Core Shell)"
  "EdenTreaty + Auth Client"
  "Content Collections + MDX"
  "Remaining Components + Analytics"
  "HyperDX Browser SDK"
...
for g in state['groups']:
    ...
    print(f"  {icon}  Group {g['id']:>2}: {g['title']}{attempts}")
PYEOF
}
...
pending = total - done - blocked
lines = [
    "# RALPH Report — Bun Monorepo Reboot",
    "",
...
main() {
  ...
  echo ""
  echo -e "${BOLD}  RALPH Loop — Bun Monorepo Reboot${NC}"
  echo ""
