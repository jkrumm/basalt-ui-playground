...
export function GuideLayout({ frontmatter, readingTime, headings = [], prevNext, children }: GuideLayoutProps) {
          ...
          <div className={`mdx-prose ${styles.content}`}>
            ...
            {hasPrerequisites && (
              <Callout intent="primary" style={{ marginBottom: '1.5rem', marginTop: '1.5rem' }}>
                <p style={{ fontWeight: 600, marginBottom: '0.5rem', marginTop: 0 }}>Prerequisites</p>
                <p style={{ margin: 0 }}>
                ...
                <ul style={{ margin: '0.5rem 0 0', padding: '0 0 0 1.25rem' }}>
                  {frontmatter.prerequisites!.map(prereq => (
                    <li key={prereq}>
                      <Link to="/guides/$slug" params={{ slug: prereq }} style={{ color: 'inherit' }}>
                        {prereq.replaceAll('-', ' ')}
                      </Link>
                    </li>
                  ))}
            ...
            <Box marginBottom={8} marginTop={8}>
              ...
              <p className={`${Classes.TEXT_MUTED} ${Classes.TEXT_LARGE}`} style={{ marginTop: '0.75rem', marginBottom: 0 }}>
            ...
            {prevNext && (prevNext.prev || prevNext.next) && (
              <Flex justifyContent="space-between" gap={4} className={styles.prevNextNav}>
                {prevNext.prev
                  ? (
                      <Link to="/guides/$slug" params={{ slug: prevNext.prev.slug }} className={`${styles.navLink} ${styles.content}`}>
                        <Card elevation={Elevation.ONE} interactive style={{ height: '100%' }}>
                          <Flex alignItems="center" gap={3} padding={3}>
                            ...
                            <Box>
                              ...
                              <Tag minimal intent={DIFFICULTY_INTENTS[prevNext.prev.frontmatter.difficulty] ?? 'none'}>
                                {prevNext.prev.frontmatter.difficulty}
                              </Tag>
                            </Box>
                          </Flex>
                        </Card>
                ...
                {prevNext.next && (
                  <Link to="/guides/$slug" params={{ slug: prevNext.next.slug }} className={`${styles.navLink} ${styles.content}`}>
                    <Card elevation={Elevation.ONE} interactive style={{ height: '100%' }}>
                      <Flex justifyContent="end" alignItems="center" gap={3} padding={3}>
                        <Box style={{ textAlign: 'right' }}>
                          ...
                          <Tag minimal intent={DIFFICULTY_INTENTS[prevNext.next.frontmatter.difficulty] ?? 'none'}>
                            {prevNext.next.frontmatter.difficulty}
                          </Tag>
                        </Box>
